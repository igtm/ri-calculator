use aws_sdk_ec2::model::{Instance, InstanceStateName, ReservedInstanceState, ReservedInstances};
...
impl EC2Instances {
        ...
        if let Some(s) = ri.state() {
            ...
            match s {
                ReservedInstanceState::Active => {
                    reserved_active_count = ri.instance_count().unwrap_or(0)
                }
                ReservedInstanceState::Retired => {
                    reserved_expired_count = ri.instance_count().unwrap_or(0)
                }
                _ => return,
            }
            for o in &mut self.items {
                if o.product_description
                    == ri.product_description().map(|x| x.as_str()).unwrap_or("")
    ...
    pub fn to_vec(&self) -> Vec<Vec<String>> {
        return match self.view_mode {
            ...
            ViewMode::NormalizationFactor => self
                .agg_by_instance_family()
                .iter()
                .map(|x| x.to_vec())
                .collect(),
        };
    ...
    pub fn agg_by_instance_family(&self) -> Vec<EC2InstanceNormalizationFactor> {
        ...
        for item in &self.items {
            let mut found = false;
            for o in &mut ret {
                if item.instance_family() == o.instance_family
                    && item.product_description == o.product_description
                {
                    found = true;
                    o.running_count_normalization_factor +=
                        item.running_count_normalization_factor();
                    o.reserved_active_normalization_factor +=
                        item.reserved_active_normalization_factor();
                    break;
                }
            }

            if !found {
                if item.running_count_normalization_factor() == 0.0
                    && item.reserved_active_normalization_factor() == 0.0
                {
                    continue;
                }
                ret.push(EC2InstanceNormalizationFactor {
                    ...
                    reserved_active_normalization_factor: item
                        .reserved_active_normalization_factor(),
                });
            }
        }
